function v_return = tridiagSORnaslednji(M, v_star, c, alpha)
    % Torej:
    % noviV_i = zeros(n, 1)
    ...
    for i=2:n-1
        v_nov(i) = (c(i) - M(i,3) * v_star(i+1) - M(i,1) * v_nov(i-1))/ M(i,2);
    end
    % noviV_i = (c_i - Z_i * stariV_(i+1) - S_i * noviV_(i-1) )/ D_i
    
    % noviV_end = (c_end - S_end * noviV_(end-1) )/ D_end
    v_nov(end) = (c(end) - M(end,1) * v_nov(n-1))/ M(end,2);

    v_return = alpha * v_nov + (1-alpha) * v_star;